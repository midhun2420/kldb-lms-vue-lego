...
export default {
    ...
    methods: {
        ...
        FromDateValidation (checkday) {
            if (checkday) {
                if (parseInt(checkday) < 280) {
                    // this.$notify('Enter valid days.', 'Warning!', { type: 'warning' });
                    return 'Days should be 280 or greater!';
                } else {
                    return true;
                }
            }
        ...
        async downloadReport () {
            try {
                this.loading = true;
                const response = await axios.form(urls.reports.cows.dueForDrying, this.model);
