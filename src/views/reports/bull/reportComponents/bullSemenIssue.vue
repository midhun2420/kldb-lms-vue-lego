    ...
    <div class="row">
        ...
        <div class="col-12 mt-3">
            <s-form @submit="downloadReport" class="row">
                ...
                <div class="col-lg-4">
                    <validated-ajax-vue-select :url="rsbOptionsURL" label="RSB" multiple
                                               placeholder="RSB"
                                               @input="loadData"
                                               :rules="{required:true}"
                                               class="c-input-select text-black mb-0 field-required" v-model="model.rsb"
                                               />
                </div>
                <div class="col-lg-4">
                    <validated-select :options="bullOptions" label="Animal No" multiple
                                               placeholder="Bull"
                                      :rules="{required:true}"
                                               class="c-input-select text-black mb-0 field-required" v-model="model.bulls"
                                               />
                </div>
                <div class="col-lg-2">
...
export default {
    ...
    data () {
        return {
            ...
            bullOptions: [],
            // rsbOptionsURL: urls.reports.optionURLS.rsbSelect,
    ...
    methods: {
        ...
        async loadData () {
            this.loading = true;
            const response = await axios.get(urls.reports.optionURLS.bullRsbSelect + '?rsb=' + this.model.rsb);
        ...
        async downloadReport () {
            try {
                this.loading = true;
                const response = await axios.form(urls.reports.bulls.bullSemenIssue, this.model);
