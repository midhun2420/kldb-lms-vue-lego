 ...
 <div>
     <s-form :key="formKey" @submit="addItem">
        <div class="row fl-x fl-j-sb ml-1">
            <h2 class="head font-poppins-semibold  mt-3 text-primary">Testing of French Mini Straw</h2>
            <div>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="row mt-4">
                    <div class="col-lg-2">
                        <validated-date-picker format="DD-MM-YYYY" label="Date" :disabled-date="disabledAfterToday"
                                               class="c-input-datepicker  text-black" v-model="model.test_date" :rules="rules.common"></validated-date-picker>
                    </div><div class="col-lg-2">
<!--                        <validated-select :options="sampleNoOption" label="Sample No"-->
<!--                                               class="c-input-select text-black field-required" v-model="model.sample_no" :rules="{required:true}"></validated-select>-->
                        <validated-input label="Sample No"
                                          class="text-black" v-model="model.sample_no"></validated-input>
                    </div><div class="col-lg-2">
                        <validated-input label="Lot No"
                                               class="text-black" v-model="model.lot_no" :rules="rules.positiveOnly" type="number"></validated-input>
                    </div>
                </div>
                <div class="row mt-2">
                    <div class="col-lg-2">
                        <validated-file-input :key="fileInputKey" label-text="Upload file" class="w-10r" v-model="model.test_report"></validated-file-input>
                    </div>
                </div>
...
export default {
    ...
    data () {
        return {
            ...
            loading: false,
            URL: urls.french_mini_straw.addEdit,
            listURL: urls.french_mini_straw.list,
            fields: [
                {
                    name: 'test_date',
                    title: 'Test Date'
    ...
    methods: {
        ...
        // async loadData () {
        //     this.viewData = [];
        //     const response = await axios.get(urls.french_mini_straw.list);
        ...
        async addItem () {
            try {
                ...
                if (result.success) {
                    ...
                    this.formKey++;
                    this.loadData();
                    await this.$router.push({ path: '/testing-of-french-mini-straw/' });
                } else {
