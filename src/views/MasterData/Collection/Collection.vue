<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">COLLECTION</h4>
            <div>
        ...
        <modal ref="addRegistrationType" :no-close-on-backdrop="true" title="Collection" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Collection" :rules="{required: true}"
        ...
        <modal ref="editRegistrationType" :no-close-on-backdrop="true" title="Collection">
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Collection" :rules="{required: true}"
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.collection.addEdit,
            listURL: masterURLs.master.collection.list,
            deleteURL: masterURLs.master.collection.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify('Collection Updated Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('Collection Added Successfully', 'Success', { type: 'success' });
