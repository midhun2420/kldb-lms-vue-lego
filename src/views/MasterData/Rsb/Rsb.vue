<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">RSB</h4>
            <div>
                <btn size="sm" text="ADD" icon="fa fa-plus"
                     @click="$refs.addRsb.show();"></btn>
        ...
        <modal ref="addRsb" :no-close-on-backdrop="true" title="RSB" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="RSB" :rules="{required: true}"
        ...
        <modal ref="editRsb" :no-close-on-backdrop="true" title="RSB">
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="RSB" :rules="{required: true}"
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.rsb.addEdit,
            listURL: masterURLs.master.rsb.list,
            deleteURL: masterURLs.master.rsb.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify(' RSB edited Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('RSB Added Successfully', 'Success', { type: 'success' });
        ...
        addFormSuccess () {
            this.$refs.addRsb.close();
            this.model.name = null;
            this.$refs.table.refreshTable();
        },
        editFormSuccess () {
            this.editStatus = false;
            this.$refs.editRsb.close();
            this.$refs.table.refreshTable();
        },
        editItem (item) {
