<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">SHED</h4>
            <div>
        ...
        <div class="card mt-2">
            <div class="row m-0">
                <div class="col-12">
                    <vue-table ref="table" :fields="fields" :url="listURL" :per-page="10" search-placeholder="Shed no">
                        <template slot="animal" slot-scope="props">
                            <div>
                               <span>{{ capitalizeFirstLetter(props.rowData.animal) }}</span>
                            </div>
        ...
        <modal ref="addRegistrationType" :no-close-on-backdrop="true" title="Shed" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Name" :rules="{required: true}"
                                         class="" border-radius="1" v-model="model.name"></validated-input>
                    </div>
                    <div class="col-lg-12">
                        <validated-input label="Shed No" :rules="{required: true}"
                                         class="" border-radius="1" v-model="model.shed_no"></validated-input>
                    </div>
                    <div class="col-lg-12">
                        <validated-vue-select :options="animal"
                            label="Animal" :rules="{required: true}"
                                         class="" border-radius="1" v-model="model.animal"></validated-vue-select>
                    </div>
        ...
        <modal ref="editRegistrationType" :no-close-on-backdrop="true" title="Shed">
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Name" :rules="{required: true}"
                    ...
                    <div class="col-lg-12">
                        <validated-input label="Shed No" :rules="{required: true}"
                                         class="" border-radius="1" v-model="editingItem.shed_no"></validated-input>
                    </div>
                    <div class="col-lg-12">
                        <validated-vue-select :options="animal"
                            label="Animal" :rules="{required: true}"
                                         class="" border-radius="1" v-model="editingItem.animal"></validated-vue-select>
                    </div>
                    <div class="col-12 text-right mt-4">
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.shed.addEdit,
            listURL: masterURLs.master.shed.list,
            deleteURL: masterURLs.master.shed.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify('Shed Updated Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('Shed Added Successfully', 'Success', { type: 'success' });
        ...
        editItem (item) {
            ...
            this.editingItem.shed_no = item.shed_no;
            this.editingItem.animal = item.animal;
