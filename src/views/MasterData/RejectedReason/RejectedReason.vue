<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">REJECTED REASON</h4>
            <div>
                <btn size="sm" text="ADD" icon="fa fa-plus"
                     @click="$refs.addRecord.show();"></btn>
        ...
        <modal ref="addRecord" :no-close-on-backdrop="true" title="Rejected Reason" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Rejected Reason" :rules="{required: true}"
        ...
        <modal ref="editRecord" :no-close-on-backdrop="true" title="Rejected Reason">
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Rejected Reason" :rules="{required: true}"
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.rejectedReason.addEdit,
            listURL: masterURLs.master.rejectedReason.list,
            deleteURL: masterURLs.master.rejectedReason.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify('Rejected Reason Updated Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('Rejected Reason Added Successfully', 'Success', { type: 'success' });
