<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">SOURCE</h4>
            <div>
        ...
        <modal ref="addRegistrationType" :no-close-on-backdrop="true" title="Source" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Source" :rules="{required: true}"
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.source.addEdit,
            listURL: masterURLs.master.source.list,
            deleteURL: masterURLs.master.source.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify('Source Updated Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('Source Added Successfully', 'Success', { type: 'success' });
