<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">STORAGE</h4>
            <div>
...
<!--                                     @click="deleteItem(props.rowData.id)"/>-->
        ...
        <modal ref="addRegistrationType" :no-close-on-backdrop="true" title="Storage Type" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-vue-select :options="storage"
                            label="Storage Type" :rules="{required: true}"
                                         class="" border-radius="1" v-model="model.storage_type"></validated-vue-select>
                    </div>
                    <div class="col-lg-12">
                        <validated-input
        ...
        <modal ref="editRegistrationType" :no-close-on-backdrop="true" title="Storage Type">
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-vue-select :options="storage"
                            label="Storage Type" :rules="{required: true}"
                                         class="" border-radius="1" v-model="editingItem.storage_type"></validated-vue-select>
                    </div>
                    <div class="col-lg-12">
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.storageType.addEdit,
            listURL: masterURLs.master.storageType.list,
            deleteURL: masterURLs.master.storageType.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify('Storage Type Updated Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('Storage Type Added Successfully', 'Success', { type: 'success' });
        ...
        editItem (item) {
            ...
            this.editingItem.storage_type = item.storage_type;
