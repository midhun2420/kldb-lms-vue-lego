<template>
 <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">CSB Rooms</h4>
            <div>
                <btn size="sm" text="ADD" icon="fa fa-plus"
                     @click="$refs.addCSBRooms.show();"></btn>
        ...
        <modal ref="addCSBRooms" :no-close-on-backdrop="true" title="CSB Rooms" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="CSB Rooms" :rules="{required: true}"
        ...
        <modal ref="editCSBRooms" :no-close-on-backdrop="true" title="CSB Rooms">
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="CSB Rooms" :rules="{required: true}"
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.csbrooms.addEdit,
            listURL: masterURLs.master.csbrooms.list,
            deleteURL: masterURLs.master.csbrooms.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify(' CSB Rooms edited Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('CSB Rooms Added Successfully', 'Success', { type: 'success' });
        ...
        addFormSuccess () {
            this.$refs.addCSBRooms.close();
            this.model.name = null;
            this.$refs.table.refreshTable();
        },
        editFormSuccess () {
            this.editStatus = false;
            this.$refs.editCSBRooms.close();
            this.$refs.table.refreshTable();
        },
        editItem (item) {
