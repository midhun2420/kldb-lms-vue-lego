<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">FREQUENCY</h4>
            <div>
        ...
        <modal ref="addRegistrationType" :no-close-on-backdrop="true" title="Frequency" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Frequency" :rules="{required: true}"
                                         class="" border-radius="1" v-model="model.frequency"></validated-input>
                    </div>
        ...
        <modal ref="editRegistrationType" :no-close-on-backdrop="true" title="Frequency">
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Frequency" :rules="{required: true}"
                                         class="" border-radius="1" v-model="editingItem.frequency"></validated-input>
                    </div>
                    <div class="col-12 text-right mt-4">
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.frequency.addEdit,
            listURL: masterURLs.master.frequency.list,
            deleteURL: masterURLs.master.frequency.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify('Frequency Updated Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('Frequency Added Successfully', 'Success', { type: 'success' });
        ...
        editItem (item) {
            ...
            this.editingItem.frequency = item.frequency;
