<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">ANIMAL TYPE</h4>
            <div>
        ...
        <modal ref="addRegistrationType" :no-close-on-backdrop="true" title="Animal Type" :show-additional-buttons="true">
            <template #buttons>
            ...
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Animal Type" :rules="{required: true}"
                                         class="" border-radius="1" v-model="model.animal_type"></validated-input>
                    </div>
        ...
        <modal ref="editRegistrationType" :no-close-on-backdrop="true" title="Animal Type">
            <s-form @submit="addEditItem">
                <div class="row">
                    <div class="col-lg-12">
                        <validated-input label="Animal Type" :rules="{required: true}"
                                         class="" border-radius="1" v-model="editingItem.animal_type"></validated-input>
                    </div>
                    <div class="col-12 text-right mt-4">
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.animalType.addEdit,
            listURL: masterURLs.master.animalType.list,
            deleteURL: masterURLs.master.animalType.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify('Animal Type Updated Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('Animal Type Added Successfully', 'Success', { type: 'success' });
        ...
        editItem (item) {
            ...
            this.editingItem.animal_type = item.animal_type;
