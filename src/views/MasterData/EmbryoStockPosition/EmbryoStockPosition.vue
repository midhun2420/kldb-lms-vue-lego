<template>
    <div class="container">
        <div class="mt-5 mb-2 fl-te-c card">
            <h4 class="text-primary mb-0 font-poppins-semibold">Embryo Stock Position</h4>
            <div>
        ...
        <modal ref="addRegistrationType" :no-close-on-backdrop="true" title="Embryo Stock Position" :show-additional-buttons="true">
            <template #buttons>
...
export default {
    ...
    data () {
        return {
            addEditURL: masterURLs.master.embryoStockPosition.addEdit,
            listURL: masterURLs.master.embryoStockPosition.list,
            deleteURL: masterURLs.master.embryoStockPosition.delete,
            itemID: null,
            addLoading: false,
            editLoading: false,
            editStatus: false,
    ...
    methods: {
        ...
        async addEditItem () {
            try {
                ...
                if (result.success) {
                    if (this.editStatus) {
                        this.$notify('Stock Position Updated Successfully', 'Success', { type: 'success' });
                        this.editLoading = false;
                        this.editFormSuccess();
                    } else {
                        this.$notify('Stock Position Added Successfully', 'Success', { type: 'success' });
