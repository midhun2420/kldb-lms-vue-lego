<template>
    <div>
        <div class="row fl-x fl-j-sb">
             <h2 class="head font-poppins-semibold fs-lg-4 mt-3 text-primary ml-3">Proposal</h2>
            <div class="row mr-3 mt-3">
                <div class="mr-1"><btn class="btn btn-sm  border-r-1 bordered p-1" style="cursor: pointer" @click="AddRecord">Add Proposal</btn></div>
        ...
        <vue-table ref="table" :fields="fields" :url="listURL" :per-page="10">
            <template slot="is_disposed" slot-scope="props">
                    {{ props.rowData.is_disposed ? 'Yes' : 'No' }}
            </template>
            <template slot="actions" slot-scope="props">
                <div class="btn-group">
                    <btn text="" size="sm" icon="fa fa-pencil" color="primary"
                         @click="editItem(props.rowData)"/>
                    <btn v-if="!props.rowData.is_disposed" text="" size="sm" icon="fa fa-trash" color="danger"
                         @click="deleteItem(props.rowData.id)"/>
                </div>

            </template>
        </vue-table>
        <modal ref="editRecord" :no-close-on-backdrop="true" title="Edit Proposal" header-color="primary">
            <s-form @submit="EditRecord">
                <div class="row">
                    <validated-input label="Proposal Date" :rules="{required: true}"
                                         class="col-3" border-radius="1" v-model="editingItem.proposal_date" disabled></validated-input>
                    ...
                    <validated-input label="Live Body Weight" :rules="{required: true, ...rules.positiveOnly}"
                                         class="col-3" border-radius="1" v-model="editingItem.live_body_weight"></validated-input>

                    <validated-checkbox class="col-3 mt-4" v-model="editingItem.is_disposed" label="Disposed" disabled></validated-checkbox>
...
export default {
    ...
    data () {
        return {
            ...
            deleteURL: urls.proposal.delete,
            ...
            fields: [
                ...
                {
                    name: '__slot:is_disposed',
                    sortField: 'is_disposed',
                    title: 'Disposed'
                },
                {
    ...
    methods: {
        editItem (item) {
            ...
            this.editingItem.proposal_date = item.proposal_date;
            ...
            this.editingItem.remarks = item.remarks || ''; // Set to empty string if null or undefined
            this.editingItem.is_disposed = item.is_disposed;
        ...
        AddRecord () {
            this.$router.push({ path: '/bull-proposal/' });
        },
        BackToHome () {
        ...
        //     try {
        //         const response = await axios.form(urls.proposal.addEdit, this.editingItem);
